...
public class MatchingPatientsFragmentController {
    ...
    /* PATIENT ATTRIBUTES */
    public static final String PATIENT_UUID = "uuid";
    ...
    public static final String PATIENT_PERSON_ADDRESS = "personAddress";
    public static final String PATIENT_BIRTHDATE = "birthdate";
    
    public static final String PATIENT_MPI = "mpiPatient";
    

    public static final String[] PATIENT_PROPERTIES = new String[]{PATIENT_UUID, PATIENT_GIVEN_NAME, PATIENT_FAMILY_NAME,
            PATIENT_GENDER, PATIENT_BIRTHDATE, PATIENT_PERSON_ADDRESS,PATIENT_ID};

    public static final String[] MPI_PATIENT_PROPERTIES = new String[]{PATIENT_UUID, PATIENT_GIVEN_NAME, PATIENT_FAMILY_NAME,
            PATIENT_GENDER, PATIENT_BIRTHDATE, PATIENT_PERSON_ADDRESS, PATIENT_MPI,PATIENT_ID};


    private List<SimpleObject> getSimilarOrExactPatients(AppDescriptor app,
    	...
    	List<PatientAndMatchQuality> matches = null;
    	
    	if (exactMatch) {
            matches = service.findPreciseSimilarPatients(patient, otherDataPoints, CUTOFF, determineMaxResults(app));
    	} else {
    		matches = service.findFastSimilarPatients(patient, otherDataPoints, CUTOFF, determineMaxResults(app));
    	...
    	if(! matches.isEmpty()){
    		...
    		return new LinkedList<SimpleObject>();
    	}
    }
    ...
    public List<SimpleObject> getSimilarPatients(@RequestParam("appId") AppDescriptor app,
    	...
    	return getSimilarOrExactPatients(app, service,patient,name,address,birthdateYears,birthdateMonths,request,ui,false);
    }
    ...
    public List<SimpleObject> getExactPatients(@RequestParam("appId") AppDescriptor app,
                                               ...
                                               @RequestParam(value="birthdateMonths", required = false) Integer birthdateMonths,
                                               HttpServletRequest request, UiUtils ui) throws IOException {
    	return getSimilarOrExactPatients(app, service,patient,name,address,birthdateYears,birthdateMonths,request,ui,true);
    }
    ...
    private Boolean alreadyInResults(Patient patient, List<SimpleObject> results) {
        for (SimpleObject result : results) {
            if (Integer.valueOf(result.get(PATIENT_ID).toString()).equals(patient.getId())) {
                return true;
            }
        }
        return false;
    ...
    private String [] determinePropertiesToInclude(AppDescriptor app, String[] defaultProperties) {
        ...
        if (app.getConfig().get("matchingPatientsPropertiesToDisplay") != null) {
            ...
            if (Arrays.asList(defaultProperties).contains(PATIENT_MPI)) {
                propertiesToIncludeList.add(PATIENT_MPI);
            }
    ...
    private void addRequiredPropertiesToInclude(List<String> propertiesToInclude) {
    	for(String patientProperty: PATIENT_PROPERTIES) {
    		addIfMissing(patientProperty, propertiesToInclude);
    	}
    }
